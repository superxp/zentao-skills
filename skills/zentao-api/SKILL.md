...
| 变量 | 说明 |
|------|------|
| `ZENTAO_TOKEN` | 直接指定 token，跳过登录和缓存（最高优先级） |
| `ZENTAO_URL` | 服务器地址，如 `http://zentao.example.com` |
| `ZENTAO_ACCOUNT` | 登录账号 |
| `ZENTAO_PASSWORD` | 登录密码 |

**`ZENTAO_URL` 和 `ZENTAO_ACCOUNT` 会在首次登录后写入 `~/.zentao-token.json`，后续调用无需重复设置**，只需提供 `ZENTAO_PASSWORD`（或直接设置 `ZENTAO_TOKEN`）即可。

若所有变量均缺失，提示用户并给出 `export` 命令。如果用户直接提供了服务器、账号和密码，则直接使用，但同时告知用户尽量设置为环境变量，此时为方便用户可提供一键设置的命令。

## 认证流程

所有业务 API 均需在 Header 携带 `token`。通过运行脚本 `scripts/get-token.sh` 自动处理缓存，避免每次重复登录。

脚本依赖：`curl`、`node`（Node.js）

脚本行为：

- 若缓存文件存在，自动补全缺失的 `ZENTAO_URL` 和 `ZENTAO_ACCOUNT`
- 缓存中 URL 与账号均匹配时直接复用 token（token 永久有效）
- URL 或账号变更、缓存不存在时，调用登录 API 获取新 token，并将 `token`、`url`、`account` 写入缓存
...
1. 读取环境变量；`ZENTAO_URL` 和 `ZENTAO_ACCOUNT` 若未设置会从缓存自动补全，仍缺失时提示用户
2. 运行 `scripts/get-token.sh` 获取 token（自动处理缓存，无需每次登录）
