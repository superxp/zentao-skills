...
所有业务 API 均需在 Header 携带 `token`。通过运行脚本 `scripts/get-token.sh` 自动获取 token，避免每次重复登录。

脚本依赖：`curl`、`node`（Node.js）

脚本行为：

- 若缓存文件存在，自动补全缺失的 `ZENTAO_URL`、`ZENTAO_TOKEN` 和 `ZENTAO_ACCOUNT`
- 如果用户没有提供必要信息或缓存中 URL 与账号均匹配用户给定的值时直接复用 token（token 永久有效）
...
1. 读取环境变量；`ZENTAO_URL` 和 `ZENTAO_TOKEN` 若未设置会从缓存自动补全，仍缺失时提示用户
